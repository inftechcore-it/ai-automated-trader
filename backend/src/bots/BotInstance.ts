...
export class BotInstance extends EventEmitter {
  ...
  private lastInsufficientBalanceLog = 0;
  private lastGuardrailCheck = 0;
  ...
  private async syncLiveBalance(adapter: any): Promise<void> {
    try {
      ...
      const baseBalObj = balances.find((b: any) => (b.asset || '').toUpperCase() === baseAsset);
  ...
  private async checkDexLimitOrders(tick: PriceTick): Promise<void> {
    if (this.status !== 'RUNNING') {
      return;
    }
    ...
    if (this.state.openOrders.length > 0) {
      const triggeredOrders = this.state.openOrders.filter(order => {
        ...
        if (order.side === 'BUY') return tick.price <= order.price;
        // Limit SELL triggers when market price >= limit sell price
        if (order.side === 'SELL') return tick.price >= order.price;
        return false;
      });
  ...
  private async placeOrder(action: BotAction, tick: PriceTick): Promise<void> {
    ...
    if (side === 'SELL') {
      ...
      if (!isPaper && (!holding || holding.quantity <= 0 || action.metadata?.sweepAll)) {
        try {
          const balances: any[] = await this.getUserLiveBalances();
    ...
    try {
      ...
      if ('onOrderError' in this.strategy && typeof this.strategy.onOrderError === 'function') {
        ...
        (this.strategy as any).handleError(errMsg);
      }
    }
  }
  ...
  private async getUserLiveBalances(): Promise<any[]> {
    try {
   ...
   */
  private async evaluateRAGGuardrails(): Promise<void> {
    try {
      const { ragService } = await import('../../services/ragService.js');
      ...
      if (result && result.suggested_action === 'PAUSE_BOT' && !result.safe_to_trade) {
        ...
        if (typeof (this.strategy as any).widenGrid === 'function') {
          (this.strategy as any).widenGrid(1.5);
