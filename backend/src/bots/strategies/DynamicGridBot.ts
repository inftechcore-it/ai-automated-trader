...
export class DynamicGridBot extends BaseBotStrategy {
  ...
  validate(params: BotParams): ValidationResult {
    ...
    if (maxBuysPerCoin < 1 || maxBuysPerCoin > 10) errors.push('Max buys per coin must be between 1 and 10');
    if (maxTotalBuys < 1 || maxTotalBuys > 100) errors.push('Max total buys must be between 1 and 100');
    if (maxActiveCoins < 1 || maxActiveCoins > 20) errors.push('Max active coins must be between 1 and 20');
