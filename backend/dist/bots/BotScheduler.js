...
export class BotScheduler extends EventEmitter {
    ...
    async subscribe(botId, exchange, symbol, callback) {
    ...
    unsubscribe(botId, exchange, symbol) {
        const key = this.makeKey(exchange, symbol);
        const subs = this.subscriptions.get(key);
        if (!subs)
            return;
    ...
    async startWebSocket(exchange, symbol) {
        const key = this.makeKey(exchange, symbol);
        try {
    ...
    closeWebSocket(exchange, symbol) {
        const key = this.makeKey(exchange, symbol);
        this.wsConnections.delete(key);
        this.stats.wsConnections = Math.max(0, this.stats.wsConnections - 1);
    }
    async pollPrices() {
        ...
        if (toPoll.length === 0)
            return;
        // Poll in parallel
        await Promise.allSettled(toPoll.map(async ({ exchange, symbol, key }) => {
            try {
                const adapter = await this.getAdapter(exchange);
                if (!adapter)
                    return;
...
export function getBotScheduler() {
