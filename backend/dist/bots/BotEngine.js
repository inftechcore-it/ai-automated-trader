...
export class BotEngine extends EventEmitter {
        ...
        try {
            ...
            for (const dbBot of activeBots) {
                try {
                    ...
                    await this.scheduler.subscribe(config.id, config.exchangeName, config.symbol, (tick) => instance.processTick(tick));
    ...
    async startBot(botId) {
        ...
        if (!instance) {
            ...
            // Subscribe to price feed
            await this.scheduler.subscribe(botId, config.exchangeName, config.symbol, (tick) => instance.processTick(tick));
        }
        await instance.start();
    }
    async stopBot(botId, reason = 'User requested') {
    ...
    async updateBotParams(botId, params) {
        const instance = this.bots.get(botId);
        ...
        const strategy = this.createStrategy(dbBot.strategyType);
    ...
    async getBotOrders(botId, limit = 50) {
    ...
    async getBotSnapshots(botId, limit = 168) {
    ...
    async handleStateChange(botId, status, data) {
        try {
    ...
    handleError(botId, error, severity) {
    ...
    emitLog(botId, message, level = 'info', metadata) {
        const logEntry = {
            botId,
            message,
            level,
    ...
    async getMarketPrice(exchange, symbol) {
...
export async function initializeBotEngine(socketIo) {
