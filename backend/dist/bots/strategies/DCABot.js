...
export class DCABot extends BaseBotStrategy {
    ...
    validate(params) {
        ...
        if (!amountPerBuy || amountPerBuy <= 0)
            errors.push('Amount per buy must be positive');
        if (!p.interval || !INTERVAL_MS[p.interval])
            errors.push('Invalid interval');
        if (!totalBudget || totalBudget <= 0)
            errors.push('Total budget must be positive');
        if (amountPerBuy > totalBudget)
            errors.push('Amount per buy cannot exceed total budget');
        if (p.takeProfitPercent !== undefined && takeProfitPercent <= 0)
