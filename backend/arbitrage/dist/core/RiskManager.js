...
export class RiskManager extends EventEmitter {
     ...
     */
    checkOpportunity(opportunity, requestedAmount) {
        ...
        if (this.state.consecutiveLosses >= 2)
            riskLevel = 'medium';
        if (this.state.dailyPnL < -this.config.maxDailyLossUSDT * 0.5)
            riskLevel = 'medium';
        if (opportunity.netProfitPercent < 0.2)
            riskLevel = 'medium';
        return {
            allowed: true,
     ...
     */
    openPosition(executionId, asset, exchanges, amountUSDT) {
     ...
     */
    closePosition(executionId, asset, exchanges, amountUSDT, pnl) {
...
export function createRiskManager(config) {
