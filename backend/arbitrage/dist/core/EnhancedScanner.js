...
export class EnhancedScanner extends EventEmitter {
    ...
    async discoverCommonSymbols() {
        const symbolsByExchange = new Map();
    ...
    async startWebSocketStreams() {
        for (const [name, adapter] of this.adapters) {
            try {
                const exchange = adapter.exchange;
                if (!exchange || typeof exchange.watchTicker !== 'function')
                    continue;
    ...
    async watchTicker(exchangeName, exchange, symbol) {
    ...
    updatePriceCache(exchange, symbol, data) {
    ...
    async runScan() {
        ...
        try {
            ...
            if (this.stats.scansCompleted % 10 === 0) {
                ...
                console.log(`[EnhancedScanner] Scan #${this.stats.scansCompleted}: ` +
    ...
    findOpportunityForSymbol(symbol) {
        ...
        let bestBuy = { exchange: '', ask: Infinity, data: null };
        let bestSell = { exchange: '', bid: 0, data: null };
        for (const [exchange, data] of priceList) {
            const age = now - data.timestamp;
            if (age > this.config.maxPriceAgeMs)
                continue;
        ...
        // Calculate scores
        const score = this.calculateScore(netProfitPercent, Math.min(buyLiquidityUSDT, sellLiquidityUSDT), Math.max(now - bestBuy.data.timestamp, now - bestSell.data.timestamp), history);
    ...
    calculateScore(profitPercent, liquidityUSDT, maxAgeMs, spreadHistory) {
        ...
        if (spreadHistory.length >= 5) {
            ...
            if (stdDev > 0.1)
                spreadStabilityScore = 5;
            else if (stdDev > 0.05)
                spreadStabilityScore = 10;
        }
...
export function createEnhancedScanner(adapters, config) {
