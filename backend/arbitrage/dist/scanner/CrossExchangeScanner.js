...
import { getExecutablePrice } from '../utils/orderBookUtils.js';
import { createFeeService } from '../services/FeeService.js';
...
export class CrossExchangeScanner extends EventEmitter {
    ...
    async discoverCommonPairs() {
        const pairsByExchange = new Map();
    ...
    async discoverCommonPairsLegacy() {
        const pairsByExchange = new Map();
        for (const [exchangeName, adapter] of this.adapters) {
            try {
                const exchange = adapter.exchange;
    ...
    async watchOrderBook(exchangeName, exchange, symbol) {
    ...
    updateOrderBookCache(exchange, symbol, orderBook) {
        ...
        const formatted = {
            symbol,
            exchange,
            bids: (orderBook.bids || []).slice(0, 20).map(([price, amount]) => ({ price, amount })),
            asks: (orderBook.asks || []).slice(0, 20).map(([price, amount]) => ({ price, amount })),
    ...
    async scan() {
        ...
        try {
            ...
            if (this.stats.scansCompleted % 10 === 0) {
                ...
                console.log(`[CrossExchange] Scan #${this.stats.scansCompleted}: ` +
    ...
    checkOpportunityForSymbol(symbol) {
    ...
    findOpportunitiesForSymbol(symbol) {
        const opportunities = [];
        const orderBooks = this.orderBookCache.get(symbol);
        if (!orderBooks || orderBooks.size < 2)
            return opportunities;
        ...
        let bestBuy = { exchange: '', price: Infinity, book: null, age: 0 };
        let bestSell = { exchange: '', price: 0, book: null, age: 0 };
        for (const [exchangeName, cache] of orderBooks) {
            const { orderBook, timestamp } = cache;
            const age = Date.now() - timestamp;
            // Skip stale data
            if (age > this.config.priceMaxAgeMs && !this.config.showAllMode)
                continue;
    ...
    getRecentOpportunities(limit = 20) {
        return this.recentOpportunities.slice(0, limit);
    }
    getProfitableOpportunities(minProfitPercent = 0) {
...
export function createCrossExchangeScanner(adapters, config) {
