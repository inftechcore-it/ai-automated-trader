...
export class ExecutionEngine extends EventEmitter {
    ...
    async executeCrossExchangeArbitrage(opportunity, amount) {
        ...
        try {
            ...
            if (!this.config.dryRun) {
                const buyOrder = await this.executeBuy(buyAdapter, `${opportunity.asset}/USDT`, amount / opportunity.buyPrice, opportunity.buyPrice);
            ...
            const depositAddress = await this.getDepositAddress(sellAdapter, opportunity.sellExchange, opportunity.asset);
            const withdrawLeg = {
                exchange: opportunity.buyExchange,
                action: 'withdraw',
                asset: opportunity.asset,
            ...
            if (!this.config.dryRun) {
                const withdrawal = await this.executeWithdrawal(buyAdapter, opportunity.asset, buyLeg.amount, depositAddress.address, depositAddress.tag);
            ...
            if (!this.config.dryRun) {
                const sellOrder = await this.executeSell(sellAdapter, `${opportunity.asset}/USDT`, amountAfterWithdrawal, opportunity.sellPrice);
    ...
    async executeTriangularArbitrage(opportunity, initialAmount) {
    ...
    async executeBuy(adapter, symbol, quantity, expectedPrice) {
    ...
    async executeSell(adapter, symbol, quantity, expectedPrice) {
    ...
    async getDepositAddress(adapter, exchangeName, asset, network) {
    ...
    async executeWithdrawal(adapter, asset, amount, address, tag, network) {
    ...
    async waitForDeposit(adapter, asset, expectedAmount) {
...
export function createExecutionEngine(adapters, config) {
